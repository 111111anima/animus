  ...
  async generateVideoFromImage(imageUrl: string): Promise<string> {
    try {
      ...
      const requestBody = {
        ...
        prompt: `
          Create a talking head video with these CRITICAL requirements:
          - ABSOLUTELY NO HEAD MOVEMENT WHATSOEVER (CRITICAL)
          - Head position must be 100% LOCKED in exact input position
          - ONLY mouth should move for speech animation
